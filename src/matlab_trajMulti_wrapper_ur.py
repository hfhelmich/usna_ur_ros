...
def move_callback(msg):
    ...
    try:
        client = actionlib.SimpleActionClient('/urb/scaled_pos_joint_traj_controller/follow_joint_trajectory', FollowJointTrajectoryAction)
        print("Waiting for server...")
        client.wait_for_server()
        print("Connected to server")
        print('waiting for joint states from robot')
        joint_states = rospy.wait_for_message("/urb/joint_states", JointState)
...
if __name__ == '__main__':
    ...
    traj_sub = rospy.Subscriber('/ura/joint_trajectory_MATLAB',JointTrajectory,move_callback)
    rospy.spin()
